...
import { View, Text, Image, TouchableOpacity, Alert, ScrollView, ActivityIndicator, StyleSheet } from
    'react-native';
...
const PaginaDetalle = ({ route, navigation }) => {
    ...
    const endpoint = `${uri}/productos/${route.params?.id}`; // Obtiene el ID del producto desde los
    parámetros
    ...
    const obtenerProducto = async () => {
        try {
            ...
            setCargando(false); // Cambiar el estado de carga aunque ocurra un error
        }
    };
    const actualizar = () => {
